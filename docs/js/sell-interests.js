...
let expandedSell = new Set();

export function renderSellTable(docs) {
  ...
  if (!filtered.length) {
    container.innerHTML = `<div style="padding:40px;text-align:center;color:#4a5568">${search ? "No results found." : "No sell interest submissions yet."}</div>`;
    countEl.textContent = "";
    return;
  }
  ...
  container.innerHTML = `<table>
    <thead><tr>
      ...
      <th>Property Status</th><th>Category</th><th>Date</th>
    </tr></thead>
  ...
  filtered.forEach((d, i) => {
    ...
    const isExp     = expandedSell.has(d.id);
    ...
    tr.innerHTML = `
      ...
      <td style="color:#94a3b8;font-size:13px">${d.category || "—"}</td>
      <td style="color:#94a3b8;font-size:13px">${fmtDate(d.createdAt)}</td>`;
    tr.addEventListener("click", () => { expandedSell.has(d.id) ? expandedSell.delete(d.id) : expandedSell.add(d.id); renderSellTable(docs); });
    ...
    det.innerHTML = `<td colspan="8" style="padding:0">
      <div style="background:rgba(15,17,23,0.6);border-left:3px solid #f59e0b;padding:20px 28px">
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:20px">
          ...
          <div>
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:#4a5568;margin-bottom:10px;font-weight:600">🏠 Property Details</div>
            ...
            ${dr("Building Info",   d.buildingInfo)}
            ${dr("Property Type",   Array.isArray(d.propertyType) ? d.propertyType.join(", ") : d.propertyType)}
          </div>
          <div>
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:#4a5568;margin-bottom:10px;font-weight:600">💰 Pricing</div>
