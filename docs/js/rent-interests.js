...
let expandedRent = new Set();

export function renderRentTable(docs) {
  ...
  const countEl   = document.getElementById("rent-count");
  if (!container) return;

  if (!filtered.length) {
    container.innerHTML = `<div style="padding:40px;text-align:center;color:#4a5568">${search ? "No results found." : "No rent interest submissions yet."}</div>`;
    countEl.textContent = "";
    return;
  }
  ...
  container.innerHTML = `<table>
    <thead><tr>
      ...
      <th>Tenure Type</th><th>Bedrooms</th><th>Date</th>
    </tr></thead>
  ...
  filtered.forEach((d) => {
    ...
    const isExp    = expandedRent.has(d.id);
    ...
    tr.innerHTML = `
      ...
      <td style="color:#94a3b8;font-size:13px">${d.tenureType || "—"}</td>
    ...
    tr.addEventListener("click", () => { expandedRent.has(d.id) ? expandedRent.delete(d.id) : expandedRent.add(d.id); renderRentTable(docs); });
    tbody.appendChild(tr);

    const det = document.createElement("tr");
    det.style.display = isExp ? "table-row" : "none";
    det.innerHTML = `<td colspan="8" style="padding:0">
      <div style="background:rgba(15,17,23,0.6);border-left:3px solid #a78bfa;padding:20px 28px">
