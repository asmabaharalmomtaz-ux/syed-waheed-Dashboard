...
let expandedOffplan = new Set();

export function renderOffplanTable(docs) {
  const search = (document.getElementById("offplan-search")?.value || "").toLowerCase();
  ...
  const container = document.getElementById("offplan-container");
  const countEl   = document.getElementById("offplan-count");
  if (!container) return;

  if (!filtered.length) {
    container.innerHTML = `<div style="padding:40px;text-align:center;color:#4a5568">${search ? "No results found." : "No off-plan interest submissions yet."}</div>`;
    countEl.textContent = "";
    return;
  }
  ...
  container.innerHTML = `<table>
    <thead><tr>
      ...
      <th>Project Stage</th><th>Bedrooms</th><th>Date</th>
    </tr></thead>
    <tbody id="offplan-tbody"></tbody>
  </table>`;

  const tbody = document.getElementById("offplan-tbody");
  filtered.forEach((d) => {
    const isMember = d.submittedAsMember;
    const isExp    = expandedOffplan.has(d.id);
    ...
    tr.innerHTML = `
      ...
      <td style="color:#94a3b8;font-size:13px">${d.projectStage || "—"}</td>
    ...
    tr.addEventListener("click", () => { expandedOffplan.has(d.id) ? expandedOffplan.delete(d.id) : expandedOffplan.add(d.id); renderOffplanTable(docs); });
    tbody.appendChild(tr);

    const det = document.createElement("tr");
    det.style.display = isExp ? "table-row" : "none";
    det.innerHTML = `<td colspan="8" style="padding:0">
      <div style="background:rgba(15,17,23,0.6);border-left:3px solid #f59e0b;padding:20px 28px">
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:20px">
          ...
          <div>
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:#4a5568;margin-bottom:10px;font-weight:600">🏗 Project Details</div>
            ${dr("Location",      d.location)}
            ${dr("Building",      d.building)}
            ${dr("Project Stage", d.projectStage)}
